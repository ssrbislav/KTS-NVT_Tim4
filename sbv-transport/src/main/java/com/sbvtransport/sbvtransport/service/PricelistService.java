...
public class PricelistService implements IPricelistService {
    ...
    public Pricelist getActive() {
        for (Pricelist pricelist : pricelistRepository.findAll()) {
            if (pricelist.getActive() && new Date().after(pricelist.getValid_since()) && new Date().before(pricelist.getValid_until())) {
                return pricelist;
            }
        }
        return null;
  ...
  @Override
  public double calculatePrice(TypeTransport typeTransport,
      DemographicTicketType demographicTicketType, TicketType ticketType, Zone zone) {
